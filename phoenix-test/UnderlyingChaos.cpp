...
void network_delay(int duration, int offset, string net_interface, int port, string node_id,
    HookPosition hookPosition)
{
    // std::cout << "We are now in the network_delay, ";
    int chaos_probability =
        getProbability();  // get the chaos probability controled by the coordinator
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        ...
        sprintf(network_cmd,
            "blade create network delay --time %d --offset %d --interface %s --local-port %d",
            duration, offset, net_interface.c_str(), port);
        ...
        {
            // This indicates that the json is parsed successfully and the chaos is starting
            // successfully
            string chaos_id = json_result["result"].asString();
            // write strategies to files
            writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + node_id + ".txt", std::time(0), node_id,
                hookPosition, chaos_id, network_cmd);
...
bool stop_chaos_blade_strategy(string chaos_id)
{
    ...
    bool parsingSuccessful = reader.parse(chaos_result, json_result);
    // The result is like:
    // {"code":200,"success":true,"result":{"target":"network","action":"delay","flags":{"interface":"eth0","local-port":"30300","offset":"0","time":"200"},"ActionProcessHang":false}}
    if (json_result["code"].asInt() == 200 && json_result["success"].asString() == "true")
    {
        // This indicates that the json is parsed successfully and the chaos is starting
        // successfully
...
void network_drop_packets(
    int percent, string net_interface, int port, string node_id, HookPosition hookPosition)
{
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        char network_cmd[100];
        sprintf(network_cmd,
            "blade create network loss --percent %d --interface %s --local-port %d", percent,
            net_interface.c_str(), port);
        // std::cout << "The exec cmd is " << network_cmd << std::endl;
        std::string chaos_result = executeCMD(network_cmd);
        Json::Value json_result;
        Json::Reader reader;
...
void network_reorder_packets(int correlation, int percent, string net_interface, int port,
    string node_id, HookPosition hookPosition)
{
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        char network_cmd[100];
        sprintf(network_cmd,
            "blade create network reorder --correlation %d --percent %d --interface %s  "
            "--local-port %d",
            correlation, percent, net_interface.c_str(), port);
        // std::cout << "The exec cmd is " << network_cmd << std::endl;
        std::string chaos_result = executeCMD(network_cmd);
        Json::Value json_result;
        Json::Reader reader;
        ...
        else
        {
            ...
            BOOST_THROW_EXCEPTION(
                std::runtime_error{"Network packets duplicate chaos starting error!"});
        }
    }
}

void network_corrupt_packets(
    int percent, string net_interface, int port, string node_id, HookPosition hookPosition)
{
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        char network_cmd[100];
        sprintf(network_cmd,
            "blade create network corrupt --percent %d --interface %s  --local-port %d", percent,
            net_interface.c_str(), port);
        std::cout << "The exec cmd is " << network_cmd << std::endl;
        std::string chaos_result = executeCMD(network_cmd);
        Json::Value json_result;
        ...
        else
        {
            // panic here because the chaos-blade has some errors.
            BOOST_THROW_EXCEPTION(
                std::runtime_error{"Network packets corrupt chaos starting error!"});
        }
    }
}

void network_duplicate_packets(
    int percent, string net_interface, int port, string node_id, HookPosition hookPosition)
{
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        char network_cmd[100];
        sprintf(network_cmd,
            "blade create network duplicate --percent %d --interface %s  --local-port %d", percent,
            net_interface.c_str(), port);
        // std::cout << "The exec cmd is " << network_cmd << std::endl;
        std::string chaos_result = executeCMD(network_cmd);
        Json::Value json_result;
...
void clear_block_data(string node_id, HookPosition hookPosition)
{
    ...
    if (dis(gen) < (float)(chaos_probability * 1.0 / 100.0))
    {
        ...
        case 0:
            ...
            chaos_id = "111111111";
            writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + node_id + ".txt", std::time(0), node_id,
                hookPosition, chaos_id, clear_data_cmd1);
            break;
        case 1:
            ...
            chaos_id = "222222222";
            writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + node_id + ".txt", std::time(0), node_id,
                hookPosition, chaos_id, clear_data_cmd2);
            break;
        case 2:
            ...
            chaos_id = "333333333";
            writeStrategy("/tmp/phoenix/PHOENIX_HOOK_" + node_id + ".txt", std::time(0), node_id,
                hookPosition, chaos_id, clear_data_cmd3);
            break;
        }
    }
}
...
void writeStrategy(string info_file, long int timeStamp, string node_id, HookPosition hookPosition,
    string strategy_id, string exec_cmd)
{
    ofstream MyFile;
    MyFile.open(info_file, std::ios_base::app);
    MyFile << timeStamp << "," << node_id << "," << enumToString(hookPosition) << "," << strategy_id
           << "," << exec_cmd << std::endl;
    MyFile.close();
}
...
{
    ifstream MyFile("/tmp/phoenix/" + node_id + "/" + enumToString(hookPosition));
    if (!MyFile.good())
    {
        return "WAITING";
    }
...
{
    struct stat st;
    if (stat(("/tmp/phoenix/" + node_id).c_str(), &st) != 0)
    {
        int res = system(("mkdir /tmp/phoenix/" + node_id).c_str());
    }
    ofstream MyFile("/tmp/phoenix/" + node_id + "/" + enumToString(hookPosition));
    switch (signal)
    {
    case Signal::READY:
        MyFile << "Signal:READY" << std::endl;
        break;
    case Signal::STOPCHAOS:
        MyFile << "Signal:WAITING" << std::endl;
        break;
    default:
        MyFile << "Signal:WAITING" << std::endl;
        break;
