...
string dev::aesCBCEncrypt(const string& _plainData, const string& _key)
{
    ...
    sms4_key_t sm4Key;
    sms4_set_encrypt_key(&sm4Key,(const byte*)_key.data());
    std::string ciperData;
    sms4_encrypt((const byte*)_plainData.data(), (byte*)ciperData.data(), &sm4Key);
    return ciperData;
}
...
string dev::aesCBCDecrypt(const string& _cypherData, const string& _key)
{
    ...
    sms4_key_t sm4Key;
    sms4_set_decrypt_key(&sm4Key,(const byte*)_key.data());
    std::string plainData;
    sms4_decrypt((const byte*)_cypherData.data(), (byte*)plainData.data(), &sm4Key);
    return plainData;
}
