...
bool SM2::sign(const char *_originalData, int _originalDataLen, const string &_privateKeyHex, string &_r, string &_s)
{
    bool lresult = false;
    // create EC_GROUP
    EC_GROUP *sm2Group = EC_GROUP_new_by_curve_name(NID_sm2p256v1);
    ...
    sig = SM2_do_sign((const uint8_t *)_originalData, _originalDataLen, sm2Key);
...
int SM2::verify(const string &_signData, int, const char *_originalData, int _originalDataLen, const string &_publicKeyHex)
{
    ...
    ok = SM2_do_verify((const uint8_t *)_originalData, _originalDataLen, sig, sm2Key);
      // SM2_do_verify(sm2Key, EVP_sm3(), sig, (const uint8_t *)userId,
